...
def register_hier_output(model, temb_refine):
    self = model.diffusion_model
    self.temb_refine = temb_refine
...
class UNetWrapper(nn.Module):
    def __init__(self, unet, use_attn=True, base_size=512, max_attn_size=None, attn_selector='up_cross+down_cross', temb_refine=False) -> None:
        super().__init__()
        self.unet = unet
        ...
        register_hier_output(unet, temb_refine)
...
class TextAdapter(nn.Module):
    ...
    def forward(self, latents, texts, gamma):
        ...
        texts_adapt = self.fc(texts)
        texts = texts + gamma * texts_adapt
